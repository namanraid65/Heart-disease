"""
Training Pipeline for Local 1D AlexNet Models
Trains independent local models for Hospital 1 (Cleveland), Hospital 2 (Hungarian),
...
def train_client_alexnet(
    ...
    """
    Trains a local 1D AlexNet model on a single hospital client's dataset.
    Saves the best checkpoint to models/checkpoints/<checkpoint_name>.
    """
    set_seed(seed)
    ...
    if verbose:
        print("\n" + "=" * 80)
        print(f" TRAINING LOCAL 1D ALEXNET: {client_name}")
        print("=" * 80)
    ...
    # Dynamically verify feature dimension
    sample_feat, _ = next(iter(train_loader))
    input_dim = sample_feat.shape[1]

    # 2. Build Model
    model = build_alexnet_1d(input_dim=input_dim, dropout_rate=DROPOUT_RATE).to(device)
...
def train_all_local_alexnets(seed: int = RANDOM_SEED) -> Dict[str, Dict[str, Any]]:
    """
    Trains independent local 1D AlexNet models for all 3 hospital clients.
    """
    print("=" * 80)
    print(" STARTING LOCAL 1D ALEXNET TRAINING FOR ALL CLIENTS")
    print("=" * 80)

    training_results = {}
    for cid in CLIENT_CONFIGS:
        res = train_client_alexnet(client_id=cid, verbose=True, seed=seed)
        training_results[cid] = res

    print("\n" + "=" * 80)
    print(" ALL LOCAL ALEXNET MODELS TRAINED SUCCESSFULLY!")
    print("=" * 80)
    return training_results


if __name__ == '__main__':
    train_all_local_alexnets()
